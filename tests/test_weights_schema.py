...
def test_schema_top_level_keys():
    ...
    for key in ["as_of_date", "lookback_days", "input_type", "objective", "symbols",
                "sum_weight_discrete", "cash_weight", "weights", "selected_symbols",
                "weight_semantics", "portfolio_version"]:
        assert key in payload


def test_canonical_sizing_fields():
    _, payload = _payload()
    assert payload["weight_semantics"] == "margin_fraction_of_total_equity"
    assert payload["portfolio_version"] == "mvo_bnb_v1"
    # Only valid symbols with a positive weight are selected/sized.
    assert payload["selected_symbols"] == ["BTCUSDT"]
    assert np.isclose(payload["weights"]["BTCUSDT"], 0.22)
    assert "ETHUSDT" not in payload["weights"]


def test_weights_plus_cash_sum_to_one():
    _, payload = _payload()
    total = sum(payload["weights"].values()) + payload["cash_weight"]
