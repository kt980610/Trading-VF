...
# Coverage/provenance features, present ONLY when a news provider that emits them
# (e.g. GDELT) is used. They are df-driven: absent for the no-news baseline and
# for providers that do not supply them, so they never inflate the schema with
# constant columns. ``gdelt_tone`` is the raw source tone, NOT a FinBERT score.
NEWS_PROVENANCE_FEATURES = [
    "news_available",
    "news_coverage_ratio",
    "gdelt_tone",
]
...
# Spec section 6: halving-season one-hot (sums to 1) + cycle id. Used by the new
# binary close classifier. The one-hot/id columns are passed through UNSCALED.
HALVING_SEASON_FEATURES = [
...
# Spec section 11: MDP-as-feature-engine outputs (no ADD_MARGIN action).
MDP_FEATURE_NAMES = [
...
# Spec section 5C: leakage-free intraday volume ratio features.
VOLUME_RATIO_FEATURES = [
    "current_volume_to_predicted_daily_volume",
    "current_volume_to_previous_day_daily_volume",
]

# CAPM rolling beta (per symbol, per day, D-1 join). Continuous -> scaled.
BETA_FEATURES = ["capm_beta"]

# Columns that must NEVER be scaled (one-hot / categorical codes / ids / binary).
PASSTHROUGH_COLUMNS = [
    "side_code",
    "mode_code",
    "season_pre_halving_2y",
...
# Trade-grouping / bookkeeping columns kept out of the feature matrix.
TRADE_META_COLUMNS = [
    "trade_id",
    "leg_id",
    "side_label",
    "entry_timestamp",
    "trade_pnl_rate_now",
    "best_future_trade_pnl_rate",
    "decision_value_gap",
]
CLOSE_LABEL_COLUMN = "close_label"
...
def news_like_columns(df: pd.DataFrame) -> List[str]:
    ...
    def is_news(col: str) -> bool:
        return (
            ...
            # GKG daily market-wide features (counts + tone mean/vol + coverage).
            or col.startswith("gkg_")
        )
...
def close_classifier_feature_columns(df: pd.DataFrame, include_optional: bool = False) -> List[str]:
    """Full ordered RF-close-classifier feature set present in ``df``.

    Edges + (optional components) + state + volume + volume-ratios + news +
    halving one-hot + MDP features, with raw price levels / side/mode strings
    excluded and side/mode replaced by numeric codes.
    """
    cols: List[str] = list(INTEGRAL_EDGE_FEATURES)
    if include_optional:
        cols += list(OPTIONAL_COMPONENT_FEATURES)
    cols += STATE_FEATURES + VOLUME_FEATURES + VOLUME_RATIO_FEATURES
    cols += NEWS_FEATURES + NEWS_PROVENANCE_FEATURES + HALVING_SEASON_FEATURES + MDP_FEATURE_NAMES
    cols += BETA_FEATURES
    excluded = {"side", "mode", *PRICE_EXCLUDED_FROM_ML}
    cols = [c for c in cols if c not in excluded]
    cols += ["side_code", "mode_code"]
    # De-dup preserving order, keep only columns actually present.
    seen, ordered = set(), []
    for c in cols:
        if c in df.columns and c not in seen:
            seen.add(c)
            ordered.append(c)
    extras = [c for c in news_like_columns(df) if c not in seen]
    return ordered + extras


def close_classifier_column_split(df: pd.DataFrame, include_optional: bool = False):
    """Return ``(feature_cols, scale_cols, passthrough_cols)`` for the classifier."""
    feature_cols = close_classifier_feature_columns(df, include_optional)
    passthrough = [c for c in feature_cols if c in set(PASSTHROUGH_COLUMNS)]
    scale_cols = [c for c in feature_cols if c not in set(PASSTHROUGH_COLUMNS)]
    return feature_cols, scale_cols, passthrough
