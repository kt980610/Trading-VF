...
def _simulate_one(args) -> pd.DataFrame:
    ...
    rf_policy, status = rfc.load_policy(promoted_dir)
    model_promoted = rf_policy is not None
    if rf_policy is None:
        print(f"{symbol} status={status} -> baseline (no cross-symbol fallback)")
        rf_policy = rfm.BaselineModel(threshold=config.simulation.decision_threshold)
    baseline_policy = rfm.BaselineModel(threshold=config.simulation.decision_threshold)

    def run(policy):
        # MDP add-margin is disabled in live/eval action logic (spec section 11);
        # MDP contributes features only.
    ...
    out["reason_if_not_promoted"] = "" if model_promoted else "missing_symbol_model"
    return out
