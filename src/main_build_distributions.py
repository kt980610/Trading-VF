...
if __package__ in (None, ""):
    ...
    from src.data_loader import DataSourceError, _daily_path, load_daily_ohlcv
    ...
    from .data_loader import DataSourceError, _daily_path, load_daily_ohlcv
...
def run(config_path: str) -> dict:
    ...
    for symbol in config.symbols:
        try:
            df = load_daily_ohlcv(config, symbol)
        except DataSourceError as exc:
            symbols_out[symbol] = {"valid": False, "reason": exc.reason}
            print(f"{symbol} skipped reason={exc.reason}")
            continue
        if df is None:
            symbols_out[symbol] = {"valid": False, "reason": "missing_daily_source"}
            print(f"{symbol} skipped reason=missing_daily_source")
            continue

        # PDFs derive ONLY from daily percent change (return_decimal), never price.
        returns = compute_daily_returns(df["close"])
        entry = build_symbol_entry(config, returns)
        if isinstance(entry, dict):
            entry["source_file"] = os.path.relpath(_daily_path(config, symbol))
